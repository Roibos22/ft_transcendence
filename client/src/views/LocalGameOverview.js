...
export class LocalGameOverview {
	...
	async init() {
		...
		this.content.nextMatchPlayers = document.getElementById('nextMatchPlayers');
		this.content.goToNextMatch = document.getElementById('goToNextMatch');
	...
	async initNextMatch() {
		...
		await this.getLocalGame(currentMatch);
		this.updateNextMatchDisplay(currentMatch);
	}
	...
	async getLocalGame(currentMatch) {
		...
		this.initGameSocket(data.game_id, currentMatch);
	}

	initGameSocket(gameId, currentMatch) {
	...
	updateNextMatchDisplay(match) {
		this.content.nextMatchPlayers.textContent = `${match.players[0].name} vs ${match.players[1].name}`;
	}

	addEventListeners() {
        this.content.goToNextMatch.addEventListener('click', (e) => {
			e.preventDefault();
